...
const LeetCodeStats = ({ username }) => {
  ...
  const overallProgress = stats.totalQuestions > 0 ? 
    ((stats.totalSolved / stats.totalQuestions) * 100).toFixed(1) : 0;
        ...
        {activeTab === 'overview' && (
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {[
                { name: 'Easy', data: stats.easy, color: 'emerald', bgColor: 'from-emerald-500/20 to-green-500/20' },
                { name: 'Medium', data: stats.medium, color: 'amber', bgColor: 'from-amber-500/20 to-yellow-500/20' },
                { name: 'Hard', data: stats.hard, color: 'red', bgColor: 'from-red-500/20 to-pink-500/20' }
              ].map((difficulty) => (
                <div key={difficulty.name} className={`bg-gradient-to-br ${difficulty.bgColor} rounded-xl p-6 border border-slate-600/30`}>
                  <div className="text-center">
                    <div className={`text-3xl font-bold text-${difficulty.color}-400 mb-2`}>
                      {difficulty.data.solved}
                    </div>
                    <div className="text-slate-300 font-medium mb-1">{difficulty.name}</div>
                    <div className="text-slate-500 text-sm mb-4">of {difficulty.data.total}</div>
              ...
              <div key={category.name} className="bg-slate-800/30 rounded-xl p-6 border border-slate-700/30">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className={`bg-gradient-to-r ${category.gradient} text-white px-3 py-1 rounded-full text-sm font-bold`}>
                    {category.data.solved} / {category.data.total}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="text-center">
                    ...
                    <div className={`text-2xl font-bold text-${category.color}-400`}>{category.data.solved}</div>
                  </div>
                  <div className="text-center">
                    <div className="text-slate-400 text-sm mb-1">Submissions</div>
                    <div className="text-2xl font-bold text-slate-300">{category.data.submissions}</div>
                  </div>
                  <div className="text-center">
                    <div className="text-slate-400 text-sm mb-1">Acceptance</div>
                    <div className={`text-2xl font-bold text-${category.color}-400`}>{category.data.acceptanceRate}%</div>
                  </div>
                </div>
                ...
                <div className="mt-4">
                  <div className="w-full bg-slate-700 rounded-full h-2">
                    <div 
                      ...
                      style={{ width: `${category.data.progress}%` }}
                    ></div>
                  </div>
                </div>
              </div>
            ...
            {stats.badges && stats.badges.length > 0 && (
              <div className="bg-slate-800/30 rounded-xl p-6 border border-slate-700/30">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                  {stats.badges.slice(0, 8).map((badge, index) => (
                    <div key={badge.id || index} className="flex items-center space-x-3 bg-slate-700/30 rounded-lg p-3">
                      {badge.icon && (
